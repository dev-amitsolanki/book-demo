...
const sections = ref([]);
const book_name = ref(props.book.name);
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800">Edit Book
                    ...
                    <form @submit.prevent="form.patch(route('books.update',book.id))">
                        ...
                        <div v-for="(data,index) in form.chapters" :key="index"> 
                            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg mt-3">
                                ...
                                <Section :data="data" :index="index" @add-sub-section="addSubSection" :book="book" ></Section>
                            </div>
                        </div>
